...
// Slices at/above this % get an outward callout ON the donut; thinner ones carry no
// external label and appear only in the trimmed legend below (zero duplication).
...
// Save-as-PDF / Print flow. Slices ≥ DONUT_LABEL_MIN% get an OUTWARD callout (name /
// amount · %) with a short leader line; thinner slices appear only in the trimmed legend
// below. Deliberately SMALL radius + tight label columns + compact font so the longest
// names ("Transportation" / "Accommodation") sit fully inside the 400-wide viewBox at
// ~390px screens — labels are placed within x∈[0,400], so no letter can clip.
function donutSVG(slices, centerTop, centerSub, ink, inkDim, sym = "$") {
  const W = 400, H = 240, cx = W / 2, cy = H / 2, rOuter = 56, rInner = 36;
  const LABEL_MIN = DONUT_LABEL_MIN;
  const NAME_FS = 12, DETAIL_FS = 11, LINE_DY = NAME_FS + 1;
  ...
  const GAP = Math.round(NAME_FS * 0.72 + LINE_DY + DETAIL_FS * 0.25 + 6), TOP = 14, BOT = H - 14;
  const layout = (arr) => {
  ...
  let labels = "";
  const draw = (l) => {
    const colX = l.right ? cx + rOuter + 12 : cx - rOuter - 12;   // tight label column, close to the ring
    const [p0x, p0y] = pt(rOuter, l.mid), [p1x, p1y] = pt(rOuter + 6, l.mid);   // short leader stub
  ...
  // Auto-fit the center total to the (small) inner circle.
  const ctStr = String(centerTop);
  const innerW = rInner * 2 * 0.86;
  const centerFS = Math.max(10, Math.min(20, Math.round(innerW / (ctStr.length * 0.60))));
  return `<svg viewBox="0 0 ${W} ${H}" width="100%" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Spending by category" style="max-width:400px;display:block;margin:0 auto;height:auto">${wedges}<text x="${cx}" y="${cy - 1}" text-anchor="middle" font-size="${centerFS}" font-weight="900" fill="${ink}">${esc(centerTop)}</text><text x="${cx}" y="${cy + 13}" text-anchor="middle" font-size="10" font-weight="600" fill="${inkDim}">${esc(centerSub)}</text>${labels}</svg>`;
}
  ...
  return (
    <div style={{ padding: "20px 16px 100px" }}>
      ...
      {donut.slices.length > 0 && (
        <Card style={{ marginBottom: 16, padding: "14px 8px" }}>
          <div style={{ color: T.textMid, fontSize: 12, fontWeight: 700, textTransform: "uppercase", marginBottom: 10, padding: "0 6px" }}>Spending Breakdown</div>
          <div dangerouslySetInnerHTML={{ __html: donutSVG(donut.slices, donutCenter, "spent", T.text, T.textDim, curByCode(tc).symbol) }} />
          {/* Thin slices (< DONUT_LABEL_MIN%) carry no on-donut callout — list them below. */}
          {donut.slices.some(s => s.pct < DONUT_LABEL_MIN) && (
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12, padding: "0 6px" }}>{donut.slices.filter(s => s.pct < DONUT_LABEL_MIN).map(s => <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14 }}><span style={{ width: 12, height: 12, borderRadius: 3, background: s.color, flexShrink: 0 }} /><span style={{ flex: 1, minWidth: 0, color: T.textMid, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.label}</span><span style={{ color: T.text, fontWeight: 800, flexShrink: 0 }}>{fmtCur(s.value, tc)}</span><span style={{ width: 44, textAlign: "right", color: T.textMid, fontWeight: 800, flexShrink: 0 }}>{Math.round(s.pct)}%</span></div>)}</div>
          )}
        </Card>
      )}
