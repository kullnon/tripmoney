      ...
      {legs.map((leg, i) => {
        ...
        <Card key={leg.id} style={{ marginBottom: 12, borderColor: LEG_COLORS[i % LEG_COLORS.length] + "44", background: LEG_COLORS[i % LEG_COLORS.length] + "08" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
            ...
            <div><div style={{ color: T.textDim, fontSize: 10, fontWeight: 600, marginBottom: 4 }}>{i === legs.length - 1 ? "ARRIVE HOME" : "LEAVE TO"}</div><input type="date" value={leg.returnDate} onChange={e => updateLeg(leg.id, "returnDate", e.target.value)} style={{ ...inputStyle, padding: "10px 12px", fontSize: 13 }} /></div>
          </div>
