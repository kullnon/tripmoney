...
function generateReportHTML(trip, expenses) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}body{font-family:'DM Sans',sans-serif;background:#fff;color:#1a1a2e;padding:40px;max-width:800px;margin:0 auto}
    ...
    <div class="footer">Generated by TripMoney · ${new Date().toLocaleDateString()} · ${trip.currency} base currency</div>
  </body></html>`;
}
...
function EmailReportScreen({ trip, expenses, onBack }) {
  ...
  const handleDownload = () => { const html = generateReportHTML(trip, expenses); const blob = new Blob([html], { type: "text/html" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `TripMoney-${trip.name.replace(/\s+/g, "-")}-Report.html`; a.click(); URL.revokeObjectURL(url); };
...
function WelcomeScreen({ onStart, onCreateTrip }) {
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
      <img src="/logo.png" alt="TripMoney" style={{ width: 80, height: 80, marginBottom: 16, borderRadius: 16 }} onError={e => { e.target.style.display = "none"; }} />
